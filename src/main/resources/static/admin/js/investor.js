...
function createPage(paramPagenum) {
  ...
  callAPIGetAllInvestors(vQueryParamsObj); 
}
  ...
  if (vCheck) {
    switch (gStatus) {
      case gStatusEnum.CREATE:
        callAPICreateInvestor(accessToken, vLayoutObj);
        break;
      case gStatusEnum.UPDATE:
        callAPIUpdateInvestor(accessToken, gId, vLayoutObj);
        break;
    }
  }
}
...
//Call API
function callAPIGetAllInvestors(paramQueryObj) {
...
function callAPICreateInvestor(paramAccessToken, paramDataObj) {
  ...
  $.ajax({
    type: "POST",
    headers: headers,
    url: gBASE_URL + `/address-maps/${paramDataObj.addressId}/investor`,
    contentType: "application/json",
    data: JSON.stringify(paramDataObj),
    success: function (paramData) {
      showToast(1, "Create Investor successfully");
...
function callAPIUpdateInvestor(paramAccessToken, paramId, paramDataObj) {
  ...
  $.ajax({
    type: "PUT",
    headers: headers,
    url: gBASE_URL + `/address-maps/${paramDataObj.addressId}/investor/` + paramId,
    contentType: "application/json",
    data: JSON.stringify(paramDataObj),
    success: function (paramData) {
      showToast(1, "Update Investor successfully");
      $('#main-modal').modal('hide');
      createPage(1);
    },
    error: function(error) {
...
function callAPIDeleteInvestor(paramId, paramAccessToken) {
  ...
  $.ajax({
    ...
    success: function (res) {
      $("#main-modal").modal("hide");
      showToast(1, "Delete Investor successfully!");
      createPage(1);
    },
    error: function(error) {
      handleError(error);
