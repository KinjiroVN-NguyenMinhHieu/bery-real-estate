...
const gSTT_COL = 0;
const gNAME_COL = 1;
const gCONTACT_TITLE_COL = 2;
const gADDRESS_COL = 3;
const gEMAIL_COL = 4;
const gMOBILE_COL = 5;
const gCREATED_BY_COL = 6;
const gUPDATED_BY_COL = 7;
...
var gTable = $("#main-table").DataTable({
  ...
  columns: [
    ...
    { data: gCOLS[gCONTACT_TITLE_COL] },
    { data: gCOLS[gADDRESS_COL] },
    { data: gCOLS[gEMAIL_COL] },
    { data: gCOLS[gMOBILE_COL] },
    { data: gCOLS[gCREATED_BY_COL] },
    { data: gCOLS[gUPDATED_BY_COL] },
...
function createPage(paramPagenum) {
  ...
  callAPIGetAllCustomers(vQueryParamsObj); 
}
...
function onIconApproveClick(paramIcon) {
  ...
  callAPIGetProjectById(accessToken, gId);
}
...
function onBtnConfirm() {
  ...
  if (gStatus === gStatusEnum.DELETE) {
    callAPIDeleteCustomer(gId, accessToken);
    return;
  }
  ...
  //B2: Validation
  let vCheck = validateDataSubmit(vCustomerObj);
  //B3: Call Api
  if (vCheck) {
    switch (gStatus) {
      case gStatusEnum.CREATE:
        callAPICreateCustomer(accessToken, vCustomerObj);
        break;
      case gStatusEnum.UPDATE:
        callAPIUpdateCustomer(accessToken, gId, vCustomerObj);
        break;
    }
  }
}
...
//Call API
function callAPIGetAllCustomers(paramQueryObj) {
  ...
  $.ajax({
    url: gBASE_URL + "/customers?" + vQueryParams.toString(),
    headers: headers,
    method: "GET",
...
function callAPIGetProjectById(paramAccessToken, paramId) {
  //Khai báo xác thực ở headers
  let headers = {
...
function callAPICreateCustomer(paramAccessToken, paramEmployeeObj) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    success: function (paramData) {
      showToast(1, "Create Customer successfully");
...
function callAPIUpdateCustomer(paramAccessToken, paramId, paramEmployeeObj) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    data: JSON.stringify(paramEmployeeObj),
    success: function (paramData) {
      showToast(1, "Update Customer successfully");
      $('#main-modal').modal('hide');
      createPage(1);
    },
    error: function(error) {
...
function callAPIDeleteCustomer(paramId, paramAccessToken) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    success: function (res) {
      $("#main-modal").modal("hide");
      showToast(1, "Delete Customer successfully!");
      createPage(1);
    },
    error: function(error) {
        handleError(error);
...
function handleLoadDataToForm(paramData) {
  $("#input-name").val(paramData.contactName);
  $("#input-title").val(paramData.contactTitle);
  $("#input-email").val(paramData.email);
  $("#input-mobile").val(paramData.mobile);
...
function collectDataSubmit(paramEmployeeObj) {
  paramEmployeeObj.contactName = $.trim($("#input-name").val());
  paramEmployeeObj.contactTitle = $.trim($("#input-title").val());
  paramEmployeeObj.address = $.trim($("#input-address").val());
  paramEmployeeObj.email = $.trim($("#input-email").val());
  paramEmployeeObj.mobile = $.trim($("#input-mobile").val());
  paramEmployeeObj.note = $.trim($("#input-note").val());
}

// Hàm validation
function validateDataSubmit(paramEmployeeObj) {
  if (!paramEmployeeObj.contactName || paramEmployeeObj.contactName.trim() === "") {
    showToast(3, "Contact Name is invalid");
    return false;
  }

  if (!paramEmployeeObj.mobile || paramEmployeeObj.mobile.trim() === "") {
    showToast(3, "Mobile is invalid");
    return false;
  }
