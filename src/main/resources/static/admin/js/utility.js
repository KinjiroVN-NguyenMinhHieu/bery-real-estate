...
function createPage(paramPagenum) {
  ...
  callAPIGetAllUtilities(vQueryParamsObj); 
}
...
function onIconApproveClick(paramIcon) {
  ...
  callAPIGetUtilityById(accessToken, gId);
}
...
function onBtnConfirm() {
  ...
  if (gStatus === gStatusEnum.DELETE) {
    callAPIDeleteUtility(gId, accessToken);
    return;
  }
  ...
  if (vCheck) {
    switch (gStatus) {
      case gStatusEnum.CREATE:
        callAPICreateUtility(accessToken, vDataObj);
        break;
      case gStatusEnum.UPDATE:
        callAPIUpdateUtility(accessToken, gId, vDataObj);
        break;
    }
  }
}
...
//Call API
function callAPIGetAllUtilities(paramQueryObj) {
  ...
  $.ajax({
    url: gBASE_URL + "/utilities?" + vQueryParams.toString(),
    headers: headers,
    method: "GET",
...
function callAPIGetUtilityById(paramAccessToken, paramId) {
  //Khai báo xác thực ở headers
  let headers = {
  ...
  $.ajax({
    url: gBASE_URL + "/utilities/" + paramId,
    headers: headers,
    method: "GET",
...
function callAPICreateUtility(paramAccessToken, paramDataObj) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    success: function (paramData) {
      showToast(1, "Create Utility successfully");
...
function callAPIUpdateUtility(paramAccessToken, paramId, paramDataObj) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    data: JSON.stringify(paramDataObj),
    success: function (paramData) {
      showToast(1, "Update Utility successfully");
      $('#main-modal').modal('hide');
      createPage(1);
    },
    error: function(error) {
...
function callAPIDeleteUtility(paramId, paramAccessToken) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    success: function (res) {
      $("#main-modal").modal("hide");
      showToast(1, "Delete Utility successfully!");
      createPage(1);
    },
    error: function(error) {
        handleError(error);
