...
var gTable = $("#main-table").DataTable({
  ...
  columns: [
    ...
    { data: gCOLS[gWEBSITE_COL] },
    { data: gCOLS[gNOTE_COL] },
...
function onPageLoading() {
  ...
  createPage(1);
  callAPIGetAllAddress(accessToken);
}
...
function createPage(paramPagenum) {
  ...
  callAPIGetAllDesignUnits(vQueryParamsObj); 
}
...
function onIconApproveClick(paramIcon) {
  ...
  callAPIGetDesignUnitById(accessToken, gId);
}
...
function onBtnConfirm() {
  ...
  if (gStatus === gStatusEnum.DELETE) {
    callAPIDeleteDesignUnit(gId, accessToken);
    return;
  }
  ...
  if (vCheck) {
    switch (gStatus) {
      case gStatusEnum.CREATE:
        callAPICreateDesignUnit(accessToken, vLayoutObj);
        break;
      case gStatusEnum.UPDATE:
        callAPIUpdateDesignUnit(accessToken, gId, vLayoutObj);
        break;
    }
  }
}
...
//Call API
function callAPIGetAllDesignUnits(paramQueryObj) {
  ...
  blockUI();
  $.ajax({
    url: gBASE_URL + "/design-units?" + vQueryParams.toString(),
    headers: headers,
    method: "GET",
...
function callAPIGetAllAddress(paramAccessToken) {
  //Khai báo xác thực ở headers
  let headers = {
...
function callAPIGetDesignUnitById(paramAccessToken, paramId) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  blockUI();
  $.ajax({
    url: gBASE_URL + "/design-units/" + paramId,
    headers: headers,
    method: "GET",
...
function callAPICreateDesignUnit(paramAccessToken, paramDataObj) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    type: "POST",
    headers: headers,
    url: gBASE_URL + `/address-maps/${paramDataObj.addressId}/design-units`,
    contentType: "application/json",
    data: JSON.stringify(paramDataObj),
    success: function (paramData) {
      showToast(1, "Create Design Unit successfully");
...
function callAPIUpdateDesignUnit(paramAccessToken, paramId, paramDataObj) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    type: "PUT",
    headers: headers,
    url: gBASE_URL + `/address-maps/${paramDataObj.addressId}/design-units/` + paramId,
    contentType: "application/json",
    data: JSON.stringify(paramDataObj),
    success: function (paramData) {
      showToast(1, "Update Design Unit successfully");
      $('#main-modal').modal('hide');
      createPage(1);
    },
    error: function(error) {
...
function callAPIDeleteDesignUnit(paramId, paramAccessToken) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    success: function (res) {
      $("#main-modal").modal("hide");
      showToast(1, "Delete Design Unit successfully!");
      createPage(1);
    },
    error: function(error) {
      handleError(error);
...
function handleLoadDataToForm(paramData) {
  ...
  $("#select-address").val(paramData.addressId); 
  $("#input-phone").val(paramData.phone);
  $("#input-phone2").val(paramData.phone2);
  $("#input-fax").val(paramData.fax);
  $("#input-email").val(paramData.email);
  $("#input-website").val(paramData.website);
  $("#input-note").val(paramData.note);
}

function loadDataToAddressSelect(data) {
  for (let i = 0; i < data.length; i++) {
    let option = $("<option/>");
    option.prop("value", data[i].id);
    option.prop("text", data[i].address);
...
function collectDataSubmit(paramLayoutObj) {
  // Collect data from input fields and assign to the corresponding properties of paramLayoutObj
  paramLayoutObj.name = $.trim($("#input-name").val());
  paramLayoutObj.description = $.trim($("#input-description").val());
  paramLayoutObj.addressId = $("#select-address").val() || null;
  paramLayoutObj.phone = $.trim($("#input-phone").val());
  paramLayoutObj.phone2 = $.trim($("#input-phone2").val());
  paramLayoutObj.fax = $.trim($("#input-fax").val());
  paramLayoutObj.email = $.trim($("#input-email").val());
  paramLayoutObj.website = $.trim($("#input-website").val());
  paramLayoutObj.note = $.trim($("#input-note").val());
}

// Hàm validation
function validateDataSubmit(paramObj) {
  if (!paramObj.name || paramObj.name.trim() === "") {
    showToast(3, "Name is invalid");
    return false;
  }
  if (!paramObj.addressId || paramObj.addressId.trim() === "") {
