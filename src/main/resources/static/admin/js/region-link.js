...
const gLATITUDE_COL = 5;
const gLONGTIDUDE_COL = 6;
const gACTION_COL = 7;
...
function createPage(paramPagenum) {
  ...
  callAPIGetAllRegions(vQueryParamsObj); 
}
...
function onIconApproveClick(paramIcon) {
  ...
  callAPIGetRegionById(accessToken, gId);
}
...
function onBtnConfirm() {
  ...
  if (gStatus === gStatusEnum.DELETE) {
    callAPIDeleteRegion(gId, accessToken);
    return;
  }
  ...
  if (vCheck) {
    switch (gStatus) {
      case gStatusEnum.CREATE:
        callAPICreateRegion(accessToken, vDataObj);
        break;
      case gStatusEnum.UPDATE:
        callAPIUpdateRegion(accessToken, gId, vDataObj);
        break;
    }
  }
}
...
//Call API
function callAPIGetAllRegions(paramQueryObj) {
  ...
  $.ajax({
    url: gBASE_URL + "/region-links?" + vQueryParams.toString(),
    headers: headers,
    method: "GET",
...
function callAPIGetRegionById(paramAccessToken, paramId) {
  //Khai báo xác thực ở headers
  let headers = {
  ...
  $.ajax({
    url: gBASE_URL + "/region-links/" + paramId,
    headers: headers,
    method: "GET",
...
function callAPICreateRegion(paramAccessToken, paramDataObj) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    type: "POST",
    headers: headers,
    url: gBASE_URL + "/region-links",
    contentType: "application/json",
    data: JSON.stringify(paramDataObj),
    success: function (paramData) {
      showToast(1, "Create Region successfully");
...
function callAPIUpdateRegion(paramAccessToken, paramId, paramDataObj) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    data: JSON.stringify(paramDataObj),
    success: function (paramData) {
      showToast(1, "Update Region successfully");
      $('#main-modal').modal('hide');
      createPage(1);
    },
    error: function(error) {
...
function callAPIDeleteRegion(paramId, paramAccessToken) {
  //Khai báo xác thực ở headers
  let headers = {
    Authorization: "Bearer " + paramAccessToken
  };
  ...
  $.ajax({
    ...
    success: function (res) {
      $("#main-modal").modal("hide");
      showToast(1, "Delete Region successfully!");
      createPage(1);
    },
    error: function(error) {
        handleError(error);
...
function collectDataSubmit(paramObj) {
  paramObj.name = $.trim($("#input-name").val());
  paramObj.description = $.trim($("#input-address").val());
  paramObj.photo = $.trim($("#input-photo").val());
